...
const Navbar = () => {
    ...
    // Get selected mood from URL or state
    const [selectedMood, setSelectedMood] = useState(null);
    useEffect(() => {
        if (location.pathname === '/mood' && location.state?.selectedMood) {
            setSelectedMood(location.state.selectedMood);
        } else {
            setSelectedMood(null);
        }
    }, [location]);

    // Function to get simplified mood name
    const getSimplifiedMoodName = (mood) => {
        if (!mood) return 'Mood';
        switch(mood.id) {
            case 'happy':
                return 'Happy';
    ...
    return (
        <div className={`w-full flex justify-between items-center transition-opacity duration-300 ${showNavbar ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                    ...
                    >
                        <span 
                            ...
                            style={{
                                ...
                                transform: selectedMood ? 'translateY(-100%)' : 'translateY(0)',
                                position: 'absolute',
                                left: 0,
                                right: 0
                            }}
                        ...
                        <span 
                            ...
                            style={{
                                ...
                                transform: selectedMood ? 'translateY(0)' : 'translateY(100%)',
                            }}
                        >
                            {getSimplifiedMoodName(selectedMood)}
                        </span>
                    </p>
                </div>
            </div>
